...
from read_excel_TW import read_excel_TW
...
class read_write_excel:
    ...
    def __init__(self, filename, filetype="auto"):
        ...
        self.data_type = "MIK" # default use MIK data

        if "MIK" in filetype.upper():
            self.data_type = "MIK"
        elif "TW" in filetype.upper():
            self.data_type = "TW"
        elif "SEICHITECH" in filetype.upper():
            self.data_type = "SEICHITECH"
        else:
        ...
        if self.data_type == "MIK":
            self.fd = read_excel_MIK(self.filename)
        elif self.data_type == "SEICHITECH":
            self.fd = read_excel_Seichitech(self.filename)
        else:
            self.fd = read_excel_TW(self.filename)

    def read_data_type(self):

        '''
        MIK, SEICHITECH, TW data type
        '''
    ...
    def read_linearity_error(self):
        ...
        '''
        read linearity error directly from excel
        '''

        return self.fd.read_linearity_error()
